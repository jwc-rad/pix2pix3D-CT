...
class MyDataLoader():
    def __init__(self, df, cts=('VNC','DE'), img_shape=(512,512,8), grid=(1,1,1),
        ...
        # dicom dataframe
        def slice_count(x):
            x['xx'] = x['ImagePosition(Patient)'].apply(lambda x: [n.strip() for n in ast.literal_eval(x)][-1]).astype(float)
            M = x['xx'].max()
            m = x['xx'].min()
        ...
        self.df = df.groupby(['pid','ct']).apply(slice_count).sort_values(by=['pid','ct','slice_num']).reset_index(drop=True)
